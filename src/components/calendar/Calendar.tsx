...
import { DATE_YMD } from '@/configs/constants';
        ...
        <Popover open={openCalendar} onOpenChange={setOpenCalendar}>
            <PopoverTrigger asChild>
                ...
                >
                    ...
                    {
                        selected && selected instanceof Date && format(selected, DATE_YMD)
                    }
                    {
                        (selected instanceof Array && selected.length > 0) && (
                            selected.length > 3 ?
                                `${selected.length} selected date` :
                                selected.map(selectDate => format(selectDate, DATE_YMD)).join(', '))
                    }
                    {
                        selected && isDateRange(selected) && selected.from &&
                        `${format(selected.from, DATE_YMD)} ~ ${selected.to ? format(selected.to, DATE_YMD) : ''}`
                    }
                    {
